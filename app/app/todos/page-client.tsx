...
import { toggleCompleted, bulkToggleCompleted } from "@/actions/toggle-completed"
...
export function TodosPageClient({
  ...
  function rescheduleSelectedTodos(date: Date | undefined) {
    ...
    // Send the actual request
    bulkUpdateDueDate(idsToReschedule, date?.toISOString() || null)
  }
  ...
  const handleToggleCompleted = async (id: number, completed: boolean) => {
    const result = await toggleCompleted(id, !completed)
