...
export const assignTodo = tool({
  description: 'Assign the current todo to a user',
  parameters: z.object({
    todoId: z.number().describe('The ID of the todo to assign'),
    userId: z.string().describe('The ID of the user to assign to, or "unassign" to remove assignment'),
  }),
  execute: async ({ todoId, userId }) => {
    try {
      ...
      const message = assignedToId 
        ? `AI assigned todo to user ${userId}`
        : 'AI unassigned todo'
      ...
      return {
        success: true,
        message: assignedToId ? `Assigned todo to user ${userId}` : 'Unassigned todo',
